...
app.put('/api/open-plays/:id', authenticateToken, (req, res) => {
  const { court_name, returning_players, new_players, court_fee_rev, misc_rev, base_cost, losses } = req.body;
  db.run(`UPDATE open_plays SET court_name = ?, returning_players = ?, new_players = ?, court_fee_rev = ?, misc_rev = ?, base_cost = ?, losses = ? WHERE id = ? AND username = ?`, 
    [court_name, returning_players, new_players, court_fee_rev, misc_rev, base_cost, losses, req.params.id, req.user.username], 
    function(err) {
      if (err) return res.status(500).json({ error: err.message });
